 ...
 *
 * "ramp_router" is deliberately absent from `VENDOR_MAP`; it prices against Router's OWN
 * catalog instead (see the Ramp Router section of pricing.ts). Two distinct things would
 * otherwise go wrong at once:
 *
 *   - A price lookup under a guessed vendor can be flatly wrong. Router's catalog rate is
 *     the billed rate (measured exact against the dashboard across five vendors);
 ...
 * served tier is a reported miss (see `rampRouterUnpricedTier`). Two things the response
 * cannot say are decided, not guessed: a BYOK-served call ($0 through Router) is billed at
 * the catalog rate regardless — the response is byte-identical either way and the rate
 * equals what the vendor bills directly — and no factor is ever applied on top of the
 * published rate, even for the models measured to bill off it (documented, with a
 * `markup` recommendation, in docs/ramp-router.md).
 */
...
export function extractOpenAINative(
  ...
  // The cache-write count that reaches CanonicalUsage. Zero for every surface but Router
  // — see MAPPED_DETAIL_FIELDS for why OpenAI-native keeps it out.
  let mappedCacheWrite = 0;
  if (providerHint === RAMP_ROUTER_PROVIDER) {
    // Router bills an OpenAI-served cache WRITE at its catalog's `cache_write_input` rate
    // — measured 2026-09-07 against the dashboard: gpt-5.6-luna, input_tokens 4493 with
    // input_tokens_details.cache_write_tokens 4490, charged as 3 x input + 4490 x
    // cache_write + out, exactly, at both served tiers. So on THIS surface the count is
    // mapped, where for OpenAI-native it deliberately is not (Databricks metered the same
    // field at the plain input rate — see MAPPED_DETAIL_FIELDS). Same wire shape, two
    // measured billing conventions; the hint is what tells them apart. The count stays
    // INSIDE `input` (OPENAI_SHAPED_APIS), so computeCost moves it out before pricing and
    // the totalTokens guard leaves it alone. Anthropic-served Router calls never report
    // the key — their cold write is invisible here and bills at the input rate, a
    // documented limitation.
    mappedCacheWrite = extracted.cacheWrite;
    delete extras["input_tokens_details.cache_write_tokens"];
    delete extras["prompt_tokens_details.cache_write_tokens"];
