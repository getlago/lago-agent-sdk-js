...
export function extractOpenAINative(
  ...
  return makeCanonicalUsage({
    ...
    // Ramp Router stamps its own name over the request surface, so downstream code can
    // tell a gateway-routed call from a direct one. Safe, and lossless: Router
    // publishes no chat-completions surface, so `extracted.api` is always "responses"
    // for these and the dimension carries nothing the gateway name doesn't.
    //
    // Gated on the HINT, never on the model string: only the wrapper knows the
    // baseURL, and every existing test that asserts api === "responses" is a direct
    // OpenAI call that passes no hint, so this cannot reach them.
    api: providerHint === RAMP_ROUTER_PROVIDER ? RAMP_ROUTER_PROVIDER : extracted.api,
    extras,
  });
}
