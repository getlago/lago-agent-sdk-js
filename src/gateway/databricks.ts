...
// `user_agent`).
//
// Keep this in sync with `extractDatabricksLog` / `resolveDatabricksSubscription`: a
// column dropped from here reaches the adapter as absent, which every field degrades to
// zero/empty on rather than throwing — an under-billed event, silently. That coupling is
// what `the projection covers every column the extraction reads` pins.
...
export class DatabricksSource {
  ...
  async readUsage(
    ...
    // No ORDER BY: nothing downstream reads the row order — the BYOK join is keyed, the
    // unbilled report is sorted, and each event's `transaction_id` derives from the row's
    // own ids — so it only buys the warehouse a sort over the widest read this module makes.
    const usage = await this.query(`
            SELECT ${USAGE_COLUMNS.join(", ")} FROM system.ai_gateway.usage
