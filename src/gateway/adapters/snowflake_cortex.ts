...
// null — `defaultSubscription` becomes dead code, and every untagged row bills to a
...
// Tag-or-null is also exactly what the Databricks resolver does, for the same reason.
// Accounts that really do map one role or one user to one customer opt in by passing
// the order explicitly.
const DEFAULT_SUBSCRIPTION_ORDER = ["query_tag"] as const;
...
// Every attribution source `resolveSnowflakeSubscription` knows how to read. Named
...
export type SnowflakeSubscriptionSource = "query_tag" | "role_names" | "user_id";
...
//   IS_COMPLETED  handed over rather than acted on — see the doc comment. Measured: an
 ...
 * Driven live 2026-08-26: one query ran 200 `AI_COMPLETE` calls then held itself open for
 * 900s, with the whole view polled every 45s. Across 19 polls covering all 937s it ran, NO
 * ROW EXISTED. The row appeared 141s AFTER the query completed, already `true`, with final
 * METRICS and CREDITS, and did not move over the next six minutes. Snowflake documents
 * "running queries are updated every 2 minutes (best effort), SLA 5 minutes"; that did not
 * happen with three times the SLA to observe it. So usage is invisible until its query
 * ends, then lands complete — the RESTATEMENT this flag was feared to signal did not
 * reproduce, and rows already billed are not rewritten underneath us.
 *
 ...
 * bucket, so the multi-window case is untested — and there the flag is the smaller half of
 * the problem, because `QUERY_ID` stops being unique and an idempotency key built from it
 * collides. That is a caller's rule, not a pure extractor's, which is why the flag is
 * handed over rather than acted on here.
 */
 ...
 *
 * `order` names the sources to try, first hit wins. Default `["query_tag"]` — the tag
 * alone; see `DEFAULT_SUBSCRIPTION_ORDER` for the live measurement behind that. The
 * sources a caller can name:
 *
 ...
 *               tenant on accounts where one role means one customer. Opt-in only:
 ...
 *
 * An untagged row therefore resolves to null and falls to the backfill's
 * `defaultSubscription`, which is recoverable; billing a role or user id as if it were
 * a subscription is not. Do NOT invent a mapping table here — REST usage is meant to
...
/**
 * The default namespace for Snowflake-derived idempotency keys — "sfc" for
 * Snowflake Cortex. One exported constant rather than a string literal per call
 * site: the OpenAI wrapper and the reader must agree on it byte-for-byte (see
 * `snowflakeEventId`), and two literals would be free to drift apart with no
 * error anywhere — the dedup would just quietly stop matching.
 */
export const SNOWFLAKE_EVENT_ID_PREFIX = "sfc";
 ...
 * already billed: the wrapper builds it from the response's
 * `x-snowflake-request-id` header, the reader builds it from the view row's
 * `REQUEST_ID` (measured byte-identical, 2026-08-26), and `/events/batch`
 * rejects the second copy as a duplicate `transaction_id`. One implementation
 * shared by both paths, for the same reason `column()` lives here rather than
 * in the reader: two copies of a string format are free to drift, and this
 * drift would not error — every REST call in an overlapping window would just
 * bill twice, silently.
 *
 * Scoped by the RESOLVED subscription, never the row's own tag: `transaction_id`
 * is unique org-wide, so an unscoped key would permanently block the same row
 * from ever reaching a second subscription.
 */
export function snowflakeEventId(
  prefix: string,
  kind: string,
  subscription: string | null | undefined,
  rowId: string,
): string {
  return `${prefix}_${kind}_${subscription || "none"}_${rowId}`;
}
