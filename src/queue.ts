...
// accepted — which Lago reports as 422 `value_already_exist`, verified live, NOT as
// 409; 409 is in the set only as defence against an intermediary that uses it).
// Deliberately an explicit list rather than the 400-499 range, because two 4xx
// statuses mean "try again, later": 429 (rate limited) and 408 (request timeout).
...
// us to slow down.
//
// 413/415 are here for the OPPOSITE reason to 429: re-sending the same batch provably
// cannot succeed, because what makes it fail is a property OF THE BATCH (its size, its
// media type) and that is constant across retries. Treating them as transient
// re-prepended the identical batch at the head of the FIFO and backed off to 60s
// forever, blocking every event behind it until the buffer overflowed. Being
...
// instance) which already routes to the split path. They are kept for an intermediary
// in front of Lago — nginx's `client_max_body_size` genuinely does answer 413 without
// the request ever reaching Rails.
//
...
// destructive: the batch 402s, routes to `sendIndividually`, every isolated send 402s
// too, and each one is logged and dropped, so a lapsed account silently discarded every
// billable event for the whole outage with one `onError` for the lot. Measured against a
// server returning 402: 5 events in, 6 HTTP calls out, 0 recoverable. As transient they
// are held and retried instead — a lapsed account head-of-line-blocks at the 60s
// ceiling until `maxBufferSize` overflows, which is bounded, oldest-first and reported,
// and fully recoverable if the account is fixed inside the buffer window.
//
// 405/410 stay transient: they usually indicate a misrouted or retired endpoint, which
// a deploy can fix.
const PERMANENT_STATUSES: ReadonlySet<number> = new Set([400, 401, 403, 404, 409, 413, 415, 422]);
