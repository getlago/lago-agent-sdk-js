...
export class LagoSDK {
   ...
   * the subscription its `QUERY_TAG` names, falling back to `defaultSubscription`. By
   * default that tag is the ONLY source consulted: every live row carries a populated
   * `ROLE_NAMES`/`USER_ID`, so an order including them never falls through to the
   * default — it bills untagged rows to a Snowflake role or user id instead, which Lago
   * accepts for the nonexistent subscription without an error anywhere (measured,
   * 2026-08-28). Opt into `subscriptionOrder: ["query_tag", "role_names"]` (or
   * `"user_id"`) only when your account really maps that identity to a Lago
   * subscription.
   *
