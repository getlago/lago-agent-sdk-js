...
export const OPENAI_SHAPED_APIS = new Set([
  ...
  // output (128 of 169, total = input + output, 07_real_reasoning.json). cache_write
  // follows the same convention, confirmed by measurement on 2026-09-07: an
  // OpenAI-served cold write reported input_tokens_details.cache_write_tokens 4490
  // INSIDE input_tokens 4493, with total = input + output — so the write stays inside
  // input here too.
  //
  ...
  // convention for every vendor: measured 2026-09-04 (haiku, `input_tokens: 16` beside
  // `cache_read_input_tokens: 20113`, reconciled exactly against the dashboard) and
  // 2026-09-07 (an xAI model, `input_tokens: 65` beside `cache_read_input_tokens: 128`,
  // thinking inside output). Same gateway, same model, two conventions — one per
  // surface, which is why the stamp differs.
  "ramp_router",
]);
