...
// Imported from the module rather than the package index: the mapped-key set is an
// internal invariant this test pins, not part of the public API.
import { MAPPED_USAGE_KEYS } from "../../../../src/gateway/adapters/cloudflare_gateway.js";
...
//
// `extras` used to be a fixed three-key object, so any counter this adapter does not map
// was silently dropped. Not hypothetical: replaying the captured fixtures dropped
// `neurons` in 4 entries and `input_text_tokens` in 1, and a live Logs API pull also
// returns `units`, present in no fixture at all — the hand-maintained key list had
// already drifted past reality. Same contract `drift.test.ts` pins for the native
// adapters.
// ----------------------------------------------------------------------
describe("Cloudflare gateway log adapter — usage_metadata drift", () => {
  ...
  it("never shadows the poller's own billing inputs", () => {
  ...
  it.each([
    ...
    "output_tokens",
    "total_tokens",
  ])("every mapped spelling stays out of the sweep: %s", (key) => {
  ...
  it("no captured fixture loses a counter", () => {
    // The sweep against real data, not constructed entries. Iterating the fixture
    // directory rather than asserting a fixed key list is what makes this survive a
    // recapture: a new counter Cloudflare starts sending is caught by the next capture
    // run, with no test edit and no re-audit of the module docstring.
    ...
    for (const name of files) {
      ...
      const meta = entry.usage_metadata;
      if (meta === null || typeof meta !== "object" || Array.isArray(meta)) continue;
      const u = extractCloudflareLog(entry);
      const swept = (u.extras.usage_metadata ?? {}) as Record<string, unknown>;
      for (const key of Object.keys(meta as Record<string, unknown>)) {
        expect(
          MAPPED_USAGE_KEYS.has(key) || key in swept,
          `${name}: ${key} is neither mapped nor swept into extras`,
        ).toBe(true);
      }
    }
  });
