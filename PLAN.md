...
| #22  | WP3                           | `/v1/chat/completions` (stream + non-stream), virtual keys (hashed), BYOK (AES-256-GCM), admin REST, `/healthz` — built against Bifrost       |
| #23  | WP4                           | Lago-backed budget checks with TTL cache, 402 semantics, fail-open default + per-key strict                                                   |
| #24  | WP5                           | Abort-still-bills, traceability, metrics completeness, and redaction proofs                                                                   |
| #25  | WP6                           | Integration harness, adversarial suite, load smoke, `npm run verify:gateway`                                                                  |
| #26  | WP7                           | docker compose, `npm run demo`, gateway README, `DEFINITION_OF_DONE.md`                                                                       |
